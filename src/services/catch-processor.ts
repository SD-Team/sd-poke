...
import { removeBall, getBalls } from '../database/repositories/inventory.repo.js';
...
export function processCatch(
  ...
  const updatedBalls = getBalls(userId);
  ...
  if (success) {
    ...
    return { success: true, result, embeds: buildResultEmbed(result, trainerName, updatedBalls).embeds, noBalls: false };
  }
  ...
  return { success: false, result, embeds: buildResultEmbed(result, trainerName, updatedBalls).embeds, noBalls: false };
}
